        ...
        <div class="lg:col-span-4 border-r border-zinc-800/80 flex flex-col h-full bg-zinc-950/20">
            ...
            <div class="p-4 border-b border-zinc-800/80">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="chatSearch" placeholder="Buscar contacto..." class="input input-sm input-bordered w-full bg-zinc-900 border-zinc-800 pl-9 text-zinc-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 rounded-lg text-xs" />
                </div>
            </div>
                        ...
                        <div class="flex-grow min-w-0">
                            ...
                            <div class="flex items-center justify-between gap-2 mt-0.5">
                                <p class="text-xs text-zinc-450 truncate">
                                    {{ $contact['last_message'] ?? 'Sin mensajes anteriores' }}
                                </p>
                                <span class="badge badge-xs uppercase font-extrabold px-1 text-[8px] {{ $contact['role'] === 'gym_admin' ? 'bg-orange-500/15 border-orange-500/30 text-orange-400' : ($contact['role'] === 'trainer' ? 'bg-purple-500/15 border-purple-500/30 text-purple-400' : 'bg-zinc-800 border-zinc-700 text-zinc-400') }}">
                                    {{ $contact['role'] === 'gym_admin' ? 'Admin' : ($contact['role'] === 'trainer' ? 'Prof' : 'Socio') }}
                                </span>
                            </div>
                        </div>
                    </button>
                @empty
                    <div class="text-center py-8 text-xs text-zinc-650">No se encontraron contactos</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="lg:col-span-8 flex flex-col h-full bg-zinc-950/5">
            ...
            @if($activeUser)
                ...
                <div class="p-4 border-b border-zinc-800/80 bg-zinc-950/20 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-[10px] text-zinc-500">{{ $activeUser->email }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div wire:poll.3000ms="loadMessages" class="flex-grow overflow-y-auto p-6 space-y-4 flex flex-col min-h-0 bg-zinc-950/15">
                    @forelse($messages as $msg)
                        @php
                            $isMe = $msg['sender_id'] !== $activeUserId;
                        @endphp
                    ...
                    @empty
                        <div class="text-center py-20 text-zinc-600 text-xs">Comienza una nueva conversación mandando un mensaje abajo</div>
                    @endforelse
                </div>

                <!-- Input Text Box -->
                <form wire:submit.prevent="sendMessage" class="p-4 border-t border-zinc-800/80 bg-zinc-950/20 flex gap-2">
                    <input type="text" wire:model="newMessage" placeholder="Escribe un mensaje aquí..." class="input input-bordered w-full bg-zinc-950 border-zinc-800 text-zinc-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 rounded-xl text-xs flex-grow" />
            ...
            @else
                ...
                <div class="flex-grow flex flex-col items-center justify-center text-center p-8">
                    ...
                    <h3 class="text-base font-extrabold text-white">Mensajería y Soporte</h3>
                    <p class="text-xs text-zinc-550 max-w-xs mt-1.5 leading-relaxed">Selecciona un socio o instructor del panel izquierdo para comenzar a chatear en tiempo real.</p>
                </div>
            @endif
